...
export default class PostgreDAO {
  ...
  public async getFromTable<T extends Record<string, any>>(
    ...
    where: Partial<T> = {},
    select?: (keyof T)[],
    whereBetween?: { field: keyof T; start: any; end: any }
  ): Promise<T[]> {
    try {
      ...
      if (whereBetween) {
        const betweenCondition = `"${String(whereBetween.field)}" BETWEEN $${whereValues.length + 1} AND $${whereValues.length + 2}`;
        whereConditions += whereConditions ? ` AND ${betweenCondition}` : `WHERE ${betweenCondition}`;
        whereValues.push(whereBetween.start, whereBetween.end);
      }
